import { motion } from 'framer-motion'
import { isConfigured } from '../lib/supabase'

interface Props {
  onSignIn: () => void
}

export function LoginPage({ onSignIn }: Props) {
  return (
    <div className="flex flex-col items-center justify-center min-h-dvh bg-slate-900 px-6">
      <motion.div
        initial={{ opacity: 0, y: 32 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="flex flex-col items-center gap-8 w-full max-w-sm"
      >
        <div className="flex flex-col items-center gap-3">
          <span className="text-7xl">🃏</span>
          <h1 className="text-4xl font-black text-white tracking-tight">CardHouse</h1>
          <p className="text-slate-400 text-center">Swipe through cards. Build your collection.</p>
        </div>

        {!isConfigured && (
          <div className="w-full bg-amber-900/40 border border-amber-500/40 text-amber-300 text-xs rounded-lg px-4 py-3 text-left">
            <strong className="block mb-1">Supabase not connected</strong>
            Add <code className="bg-amber-900/60 px-1 rounded">VITE_SUPABASE_URL</code> and{' '}
            <code className="bg-amber-900/60 px-1 rounded">VITE_SUPABASE_ANON_KEY</code> to{' '}
            <code className="bg-amber-900/60 px-1 rounded">.env.local</code> to enable auth.
          </div>
        )}

        <motion.button
          whileHover={{ scale: 1.03 }}
          whileTap={{ scale: 0.97 }}
          onClick={onSignIn}
          className="flex items-center gap-3 w-full justify-center bg-white text-slate-900 font-semibold px-6 py-3.5 rounded-xl shadow-lg hover:bg-slate-100 transition-colors"
        >
          <GoogleIcon />
          Continue with Google
        </motion.button>
      </motion.div>
    </div>
  )
}

function GoogleIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  )
}
